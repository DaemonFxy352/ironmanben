...
export async function POST(req: NextRequest) {
  try {
    const { newLocation, reason } = await req.json();

    if (!newLocation) {
      return NextResponse.json({ error: "Missing newLocation" }, { status: 400 });
    }
    ...
    const message = `Crew meetup moved to ${newLocation}${reasonText}. See map: ironmanben.vercel.app`;
    const phones = subscribers.map((subscriber) => formatPhoneForTextlocal(subscriber.phone_e164));
    const result = await sendSMS({ message, to: phones, type: "meetup" });
